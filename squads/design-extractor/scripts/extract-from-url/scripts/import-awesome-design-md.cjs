...
const { logWritePath } = require("../lib/log-paths.cjs");
...
const TMP_ROOT = path.join(os.tmpdir(), "aiox-awesome-design-md-import");
...
async function fetchCatalogSlugs() {
  const response = await fetch(CATALOG_TREE_URL, {
    headers: { "user-agent": "aiox-design-ops" },
  });
  if (!response.ok) {
...
async function importSlug(slug, outputSlug) {
  ...
  const extended = buildExtended(slug, meta);
  const telemetry = buildTelemetry(slug, meta);
  ...
  fs.writeFileSync(path.join(outDir, "tokens-extended.json"), JSON.stringify(extended, null, 2));
  ...
  fs.writeFileSync(logWritePath(outDir, "extraction-log.yaml"), yaml.dump({
  ...
  const extractionClass = classifyRunDir(outDir, { slug: outputSlug, url: meta.source_url });
  writeExtractionClass(outDir, extractionClass);
  fs.writeFileSync(logWritePath(outDir, "telemetry.json"), JSON.stringify(annotateTelemetry(telemetry, extractionClass), null, 2));
...
function buildTelemetry(slug, meta) {
  return {
    ...
    coverage_real: false,
    measurement_mode: "synthetic_import",
    getdesign_slug: slug,
