 ...
 *   node scripts/enrich-existing.cjs aioxsquad    # one company only
...
function enrichOne(runDir) {
  ...
  if (!tokens || typeof tokens !== "object" || Array.isArray(tokens)) {
    return { runDir, skipped: true, reason: "tokens-invalid-shape" };
  }
...
function main() {
  ...
  const extractsRoot = path.join(repoRoot, "outputs", "design-ops", "url-extracts");
