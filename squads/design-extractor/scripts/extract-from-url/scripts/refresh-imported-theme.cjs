...
function isImportedRun(runDir) {
  const telemetry = readJson(logReadPath(runDir, "telemetry.json"), {});
  const log = readYaml(logReadPath(runDir, "extraction-log.yaml"), {});
...
async function main() {
  ...
  const root = path.join(repoRoot, "outputs", "design-ops", "url-extracts");
