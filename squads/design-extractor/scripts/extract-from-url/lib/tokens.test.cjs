...
test("buildSelfHostedFontFaces prefers source URL over embedded data by default", () => {
  const faces = [{
    family: "Anthropic Serif",
    weight: "400",
    style: "normal",
  ...
  assert.match(block, /https:\/\/anthropic\.com\/fonts\/x\.woff2/);
  assert.doesNotMatch(block, /data:font\/woff2;base64,AAAA/);
});

test("buildSelfHostedFontFaces can inline embedded data only when explicitly opted in", () => {
  const faces = [{
    family: "Anthropic Serif",
    weight: "400",
    style: "normal",
  ...
  const embedded = { "https://anthropic.com/fonts/x.woff2": "data:font/woff2;base64,AAAA" };
  const block = buildSelfHostedFontFaces(faces, "https://anthropic.com", ["Anthropic Serif"], embedded, { inlineEmbeddedFonts: true });
