...
const { buildRenderContract, buildSlotContract, inferExtractedArtifactType } = require("./render-contract.cjs");
...
test("render-contract: filters noisy raw button controls in favor of curated preview tokens", () => {
  const contract = buildRenderContract({
    tokens: {
      colors: { surface: "#0f0f0f", text: "#ffffff", primary: "#3ea6ff" },
      rounded: { md: "8px" },
      preview_tokens: {
        surface_bg: "#0f0f0f",
        text: "#ffffff",
        button_primary_bg: "#3ea6ff",
        button_primary_text: "#0f0f0f",
        button_primary_border: "#3ea6ff",
        button_radius: "9999px",
        button_padding_x: "16px",
        button_padding_y: "10px",
      },
    },
    extended: {
      components: {
        button: {
          bg: "transparent",
          text: "var(--t4a6da19e16bf221a, #606060)",
          radius: "50%",
          transition: "opacity,height",
          transform: "translate(-50%,50%)",
          shadow: "var(--paper-toggle-button-box-shadow, 0 1px 5px rgba(0,0,0,.4))",
          outline: "none",
        },
      },
    },
    cssScopes: parseCustomProperties(":root { --background: #0f0f0f; }"),
  });

  assert.strictEqual(contract.components.button.base.bg, "#3ea6ff");
  assert.strictEqual(contract.components.button.base.text, "#0f0f0f");
  assert.strictEqual(contract.components.button.base.radius, "9999px");
  assert.strictEqual(contract.components.button.base.shadow, undefined);
  assert.strictEqual(contract.components.button.source_filter, "noisy-css-control");
  assert.strictEqual(contract.warnings.some((w) => w.code === "component_non_flow_transform"), false);
});
...
test("render-contract: emits slot_contract from extracted HTML slots", () => {
  const contract = buildRenderContract({
    ...
    cssScopes: parseCustomProperties(":root { --background: #fff; }"),
    html: "<header></header><nav class=\"side-rail\"></nav><main></main><footer></footer>",
    breakpoints: [{ min_width: 768 }, { min_width: 1024 }],
  });

  assert.strictEqual(contract.extracted_artifact_type, "multi");
  assert.strictEqual(contract.page_role, "shell");
  assert.deepStrictEqual(Object.keys(contract.slot_contract.slots).sort(), ["footer", "header", "main", "rail"]);
  assert.deepStrictEqual(contract.slot_contract.slots.header.responsive_rules.breakpoints, ["768px", "1024px"]);
  assert.strictEqual(
    contract.slot_contract.slots.header.responsive_rules.adaptation,
    "extraction_gap(rule_inference_failed)",
  );
});

test("buildSlotContract emits explicit gap when layout slots are absent", () => {
  const slotContract = buildSlotContract({ html: "<div class=\"card\"></div>" });

  assert.deepStrictEqual(slotContract.slots, {});
  assert.strictEqual(slotContract.slot_contract_gap, "extraction_gap(no_layout_slots_detected)");
});

test("inferExtractedArtifactType distinguishes token, component, page, and multi bundles", () => {
  assert.deepStrictEqual(inferExtractedArtifactType({ tokens: { colors: { primary: "#111111" } } }), {
    type: "tokens_only",
  });
  assert.deepStrictEqual(inferExtractedArtifactType({ rawComponents: { button: {} } }), {
    type: "component",
  });
  assert.deepStrictEqual(inferExtractedArtifactType({
    slotContract: { slots: { main: {}, header: {} } },
  }), {
    type: "page",
    page_role: "home",
  });
