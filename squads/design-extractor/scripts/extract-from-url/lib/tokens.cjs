...
// Keep preview.html light: use absolute source URLs by default. Older runs may
// still pass embedded data URLs, but those are only used when explicitly opted in.
function buildSelfHostedFontFaces(fontFaces, sourceUrl, requestedFamilies, embeddedFonts, options = {}) {
  if (!Array.isArray(fontFaces) || fontFaces.length === 0) return "";
  const inlineEmbeddedFonts = options.inlineEmbeddedFonts === true;
  ...
  for (const face of fontFaces) {
    ...
    for (let i = 0; i < face.src_urls.length; i++) {
      ...
      try {
        ...
        if (inlineEmbeddedFonts && embedded) {
