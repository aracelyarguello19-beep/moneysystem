...
const DARK_CANONICAL_PATTERNS = [
  /\b(?:page|site|body)\s+background\s*:\s*`?#(?:0[0-9a-f]{5}|1[0-9a-f]{5}|2[0-9a-f]{5}|3[0-9a-f]{5})`?/i,
  /\bentire\s+visual\s+system\b.{0,120}\b(?:near|almost)[-\s]?black\s+background\b/i,
  /\bpage\s+(?:is|renders|opens)\s+(?:on|with)\s+(?:a\s+)?(?:near|almost)[-\s]?black\s+(?:background|canvas|surface)\b/i,
];

const LIGHT_CANONICAL_PATTERNS = [
  /\bbright\s+white\b.{0,80}\b(?:background|canvas|surface)\b/i,
  /\b(?:page|site|body)\s+background\s*:\s*`?(?:white|#fff(?:fff)?)`?/i,
  /\b(?:page|site|body)\s+(?:is|renders|opens)\s+(?:on|with)\s+(?:a\s+)?(?:white|bright\s+white|light)\s+(?:background|canvas|surface)\b/i,
];
...
function inferThemeFromDesignMd(markdown = "", tokens = {}) {
  ...
  const canonicalDarkHits = collectHits(text, DARK_CANONICAL_PATTERNS);
  const canonicalLightHits = collectHits(text, LIGHT_CANONICAL_PATTERNS);
  if (canonicalLightHits.length > canonicalDarkHits.length) {
    return {
      default: "light",
      confidence: "high",
      signals: canonicalLightHits.slice(0, 4).map((hit) => `design-md canonical="${hit}" → light`),
    };
  }
  if (canonicalDarkHits.length > canonicalLightHits.length) {
    return {
      default: "dark",
      confidence: "high",
      signals: canonicalDarkHits.slice(0, 4).map((hit) => `design-md canonical="${hit}" → dark`),
    };
  }
