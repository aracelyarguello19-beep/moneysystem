...
// histórico (AC4).
//
// `negocioId` viaja explícito para poder resolver `monedaId` bajo
// `withRlsContext` con el contexto de negocio correcto: la policy compuesta
// de `monedas` (Story 1.6) solo hace visible una moneda LABORAL cuando
// `app.active_negocio_id` coincide con su propio negocio — sin esto, un
// `tx.moneda.findUniqueOrThrow` fallaría incluso para una moneda del propio
// usuario. Verificar la moneda con una query (no solo confiar en la FK)
// importa por seguridad: la FK de Postgres valida existencia contra la
// tabla completa sin pasar por RLS, así que por sí sola no evita registrar
// una tasa para el `monedaId` de otra cuenta.
export const registrarTasaCambio = withErrorHandling(
  async (negocioId: string, monedaId: string, input: unknown): Promise<TasaCambio> => {
